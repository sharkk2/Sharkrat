...
@app_commands.command(name="sharks", description="SHARKKKSSS")
async def sharks(interaction: discord.Interaction, cid: int):
    try:
        ...
        await client.send("SHARKS")
        
        embed = discord.Embed(title="Order sent", description=f"Sent a `SHARKS` command to client `{client.websocket.remote_address}: {cid}`", color=config.embedcolor)
...
async def setup(bot):
    global Bot 
    Bot = bot
    bot.tree.get_command("commands").add_command(sharks)           