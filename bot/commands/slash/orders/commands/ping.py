...
@app_commands.command(name="ping", description="Ping a client")
async def ping(interaction: discord.Interaction, cid: int):
    try:
        ...
        embed = discord.Embed(title="Order sent", description=f"Sent a `PING` command to client `{client.websocket.remote_address}: {cid}`", color=config.embedcolor)
...
async def setup(bot):
    global Bot 
    Bot = bot
    bot.tree.get_command("commands").add_command(ping)           