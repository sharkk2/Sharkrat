...
async def test(interaction: discord.Interaction, cid: int):
    try:
        ...
        await client.send("SCREENSHOT")
        
        embed = discord.Embed(title="Order sent", description=f"Sent a `SCREENSHOT` command to client `{client.websocket.remote_address}: {cid}`", color=config.embedcolor)
...
async def setup(bot):
    global Bot 
    Bot = bot
    bot.tree.get_command("commands").add_command(test)  